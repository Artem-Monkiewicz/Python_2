...
if __name__ == "__main__":
    # menu = {1: '1. Zapis do pliku',
    #        2: '2. Odczyt z pliku',
    #        3: '3. Zapis do pliku csv',
    #        4: '4. Wyjdz'}
...
with SecretData([34, 12, 10, 20]) as data:
    print(data)
    a = 3 / 0
print("Po secret data")
...
#stwórz iterator który będzie przyjmowal:
- listę liczb
- liczbę x
- liczbę n
#iterator ma wypluwać liczby z tej listy które
podzielone przez x dają restę n
...
for elem in MyIterator(4,10):
    print(elem)

#%%
    
class MyIterator:
    def __init__(self, my_list, x, n):
    ...
    def __next__(self):
        ...
        if self.my_list[self.index] % self.x == self.n:
            return self.my_list[self.index]
        return self.__next__()

for elem in MyIterator([1, 2, 3, 4, 5, 6, 7], 2, 0):
    print(elem)

#%%
    
print('GENERATOR')
def my_gen(my_list, x, n):
    for elem in my_list:
        if elem % x == n:
            yield elem

gen = my_gen([1, 2, 3, 4, 5, 6, 7], 2, 0)
...
def str_to_int(func):
    def wrapper(number: str):
        if number.isdigit():
            return func(int(number))
        else:
            return "Podana liczba nie jest nturalna"
    return wrapper
...
def prime(num):
    if num == 0 or num == 1:
        return "liczba nie jest pirwsza"
    if num == 2 or prime_gen(num):
        return "liczba jest pirwsza"
...
if __name__ == '__main__':
    get_from_url('https://bilingual-kid.com/wp-content/uploads/2019/07/Bolek-i-Lolek.png', 'bolek.png')
...
def get_text_from_url(url, file_name):
    ...
    with urllib.request.urlopen(req, context=context) as f:
        if f.status == 200:
            content = f.read().decode(f.header.get_contnet_charset())
        else:
            return False
...
if __name__ == '__main__':
    ...
    get_data_from_url('https://wp.pl', 'wp.html')
...
if __name__ == '__main__':
    ...
    json_data = json.loads(get_from_url('https://api.nbp.pl/api/exchangerates/rates/a/chf?format=json'))

    zloty = float(input("Podaj wartość[PLN]"))2
